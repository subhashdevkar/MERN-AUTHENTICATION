...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between px-4 sm:px-6 items-center'>
      ...
      {isAuth===true?
      <div className='text-center group relative flex justify-center items-center bg-blue-800  text-white  size-10 rounded-full mb-2 cursor-pointer'>
        <span className='text-xl font-bold'>{userData.name?.charAt(0)?.toUpperCase()}</span>
        <ul  className='absolute hidden group-hover:block top-0 mt-10 font-normal text-sm right-0 bg-gray-400'>
